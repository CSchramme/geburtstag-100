...
function tickRotation() {
  const state = getState();
  // A welcome announcement is a transient interruption, not a manual pick -
  // let it run its course (see tickWelcomeRevert) instead of rotating away
  // from it early.
  if (state.display.scene === 'welcome') return;
...
// A check-in's welcome announcement takes over display.scene for a fixed
// window (see app/api/admin/attendance/route.js), then this reverts it back
// to whatever was showing before - unless the host already navigated away
// manually in the meantime, in which case there's nothing to undo.
function tickWelcomeRevert() {
  ...
  state.display.scene = w.previousScene || 'idle';
  w.revertAt = 0;
  state.sound.trigger = { key: 'whoosh', nonce: id(), at: Date.now() };
  persist();
  notifyChange();
